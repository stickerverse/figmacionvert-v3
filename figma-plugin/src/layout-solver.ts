...
export class ProfessionalLayoutSolver {
   ...
   */
  public analyzeLayoutIntelligence(nodeData: ElementNodeData): LayoutIntelligence {
    const cssAnalysis = this.analyzeCSSLayout(nodeData);
    const childrenAnalysis = this.analyzeChildrenDistribution(nodeData.children || []);
    ...
    const inferredLayout: InferredAutoLayoutResult = {
      ...
      primaryAxisAlignItems: this.mapJustifyContentToFigma(cssAnalysis.justifyContent),
    ...
    const confidenceScore = this.calculateConfidence(cssAnalysis, childrenAnalysis);
    const hybridStrategy = this.createHybridStrategy(nodeData, inferredLayout);
    const fallbackStrategy = this.createFallbackStrategy(nodeData, confidenceScore);
   ...
   */
  private inferLayoutMode(cssAnalysis: CSSAnalysis): "NONE" | "HORIZONTAL" | "VERTICAL" | "GRID" {
    if (cssAnalysis.display === 'flex') {
      return cssAnalysis.flexDirection === 'column' || cssAnalysis.flexDirection === 'column-reverse' 
        ? 'VERTICAL' 
        : 'HORIZONTAL';
    }
   ...
   */
  private inferPrimaryAxisSizing(cssAnalysis: CSSAnalysis, nodeData: ElementNodeData): "FIXED" | "AUTO" {
    const layout = nodeData.layout;
    if (!layout) return 'AUTO';
    
    // If width/height is explicitly set and flex container, use FIXED
    if (cssAnalysis.display === 'flex') {
      const isHorizontal = cssAnalysis.flexDirection === 'row' || cssAnalysis.flexDirection === 'row-reverse';
   ...
   */
  private inferCounterAxisSizing(cssAnalysis: CSSAnalysis, nodeData: ElementNodeData): "FIXED" | "AUTO" {
    const layout = nodeData.layout;
    if (!layout) return 'AUTO';
   ...
   */
  private mapJustifyContentToFigma(justifyContent: string): "MIN" | "CENTER" | "MAX" | "SPACE_BETWEEN" {
   ...
   */
  private mapAlignItemsToFigma(alignItems: string): "MIN" | "CENTER" | "MAX" | "BASELINE" | "STRETCH" {
    switch (alignItems) {
      case 'flex-start':
      case 'start':
        return 'MIN';
  ...
  private shouldIncludeStrokesInLayout(nodeData: ElementNodeData): boolean {
    ...
    return !!(nodeData.strokeWeight && nodeData.strokeWeight > 0 && nodeData.children && nodeData.children.length > 0);
  }
  
  /**
   * PROFESSIONAL: Create hybrid positioning strategy
   */
  public createHybridStrategy(nodeData: ElementNodeData, inferredLayout: InferredAutoLayoutResult): HybridLayoutPlan {
    ...
    for (const child of children) {
      const shouldUseAbsolute = this.shouldUseAbsolutePositioning(child, nodeData);
      ...
      if (shouldUseAbsolute) {
        ...
        constraintOptimizations.push({
          nodeId: child.layout?.id || 'unknown',
          constraints: enhancedChild.constraints!,
          reasoning: 'Element breaks Auto Layout flow - positioned absolutely with intelligent constraints'
        });
      } else {
  ...
  public calculateOptimalConstraints(
    ...
    if (Math.abs(centerX - parentCenterX) < horizontalTolerance) {
      horizontal = 'CENTER';
    } else if (childRect.x + childRect.width >= parentRect.width - horizontalTolerance) {
    ...
    if (Math.abs(centerY - parentCenterY) < verticalTolerance) {
      vertical = 'CENTER';
    } else if (childRect.y + childRect.height >= parentRect.height - verticalTolerance) {
  ...
  private parsePadding(nodeData: ElementNodeData): { top: number; right: number; bottom: number; left: number } {
  ...
  private detectLinearPattern(positions: any[]): boolean {
    ...
    const horizontallyAligned = sortedByX.every((pos, i) => 
      i === 0 || Math.abs(pos.y - sortedByX[0].y) < 20
    );
    
    // Check vertical alignment
    const verticallyAligned = sortedByY.every((pos, i) =>
      i === 0 || Math.abs(pos.x - sortedByY[0].x) < 20
    );
  ...
  private calculateConfidence(cssAnalysis: CSSAnalysis, childrenAnalysis: any): number {
  ...
  private shouldUseAbsolutePositioning(child: ElementNodeData, parent: ElementNodeData): boolean {
    const childLayout = child.layout;
    if (!childLayout) return false;
    
    // Use absolute positioning for:
    // 1. Elements with absolute/fixed position
    if (childLayout.position === 'absolute' || childLayout.position === 'fixed') return true;
  ...
  private calculateLayoutAlign(child: ElementNodeData, parent: ElementNodeData): "INHERIT" | "STRETCH" | "MIN" | "CENTER" | "MAX" {
  ...
  private calculateLayoutGrow(child: ElementNodeData, parent: ElementNodeData): number {
  ...
  private buildFrameConfiguration(inferredLayout: InferredAutoLayoutResult): FrameConfiguration {
    return {
  ...
  private createFallbackStrategy(nodeData: ElementNodeData, confidence: number): FallbackStrategy {
    if (confidence < 0.3) {
      return {
        mode: 'ABSOLUTE',
        reasoning: 'Low confidence in Auto Layout compatibility - using absolute positioning'
      };
    } else if (confidence < 0.7 && nodeData.children && nodeData.children.length > 0) {
      return {
        mode: 'MIXED',
        reasoning: 'Medium confidence - using hybrid Auto Layout with absolute positioned elements'
      };
    } else {
      return {
        mode: 'AUTO',
...
function canUseAutoLayout(node: ElementNodeData): boolean {
  ...
  try {
    const layoutIntelligence = professionalLayoutSolver.analyzeLayoutIntelligence(node);
    
    // Use professional confidence scoring
    const shouldUseAutoLayout = layoutIntelligence.confidenceScore > 0.7 && 
    ...
    console.warn('Professional layout analysis failed, falling back to basic analysis:', error);
    
    // Fallback to basic analysis
    if (!node.children || node.children.length <= 1) {
      return !!(node.autoLayout?.layoutMode && node.autoLayout.layoutMode !== "NONE");
    }
    
    const result = analyzeLayoutCompatibilityBasic(node);
 ...
 */
function applyProfessionalLayoutConfig(node: ElementNodeData, intelligence: LayoutIntelligence): void {
  ...
  node.autoLayout.layoutMode = inferredLayout.layoutMode;
  node.autoLayout.primaryAxisAlignItems = inferredLayout.primaryAxisAlignItems;
  node.autoLayout.counterAxisAlignItems = inferredLayout.counterAxisAlignItems;
  ...
  node.autoLayout.strokesIncludedInLayout = inferredLayout.strokesIncludedInLayout;
  ...
  if (node.children && hybridStrategy) {
    ...
    for (const child of node.children) {
      const autoChild = hybridStrategy.autoLayoutChildren.find(c => c === child);
      const absoluteChild = hybridStrategy.absoluteChildren.find(c => c === child);
...
function hasConsistentVerticalSpacing(
  ...
  const consistentGaps = gaps.filter(gap => 
    Math.abs(gap - avgGap) <= ENHANCED_POSITION_TOLERANCE
  ).length;
