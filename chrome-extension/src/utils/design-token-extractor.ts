...
export class DesignTokenExtractor {
  ...
  async extractTokens(): Promise<DesignTokensRegistry> {
    ...
    // Extract CSS variables from :root and all elements
    this.extractCSSVariables();

    // Extract design patterns from computed styles
    this.extractComputedTokens();
  ...
  /**
   * Extract CSS custom properties (variables) from all scopes
   */
  private extractCSSVariables(): void {
    // Extract from :root
    this.extractFromElement(document.documentElement, 'root');

    // Extract from all elements with custom properties
    const allElements = document.querySelectorAll('*');
    for (let i = 0; i < allElements.length; i++) {
      const element = allElements[i];
      if (element instanceof HTMLElement) {
        this.extractFromElement(element, `element-${this.elementCounter++}`);
      }
    }
  }

  /**
   * Extract CSS variables from a specific element
   */
   ...
   */
  private processVariable(name: string, value: string, scope: string, isExplicit: boolean): void {
  ...
  /**
   * Extract design tokens from computed styles for consistency
   */
  private extractComputedTokens(): void {
    ...
    for (let i = 0; i < elements.length; i++) {
      ...
      // Track usage of CSS variables in computed properties
   ...
   */
  private trackVariableUsage(element: Element, computed: CSSStyleDeclaration): void {
    const properties = [
      'color', 'background-color', 'border-color', 'fill', 'stroke',
      'font-size', 'line-height', 'letter-spacing', 'font-weight',
      'margin', 'padding', 'gap', 'border-radius', 'width', 'height',
      'box-shadow', 'filter', 'opacity', 'z-index'
    ];
   ...
   */
  private extractVariableReferences(value: string, property: string, elementType: string): void {
  ...
  private inferTokenScopes(name: string, type: TokenType): TokenScope[] {
    ...
    if (type === 'COLOR') {
      ...
      if (nameStr.includes('radius') || nameStr.includes('rounded')) {
        ...
        scopes.push('GAP');
      } else if (nameStr.includes('width') || nameStr.includes('height') || nameStr.includes('size')) {
  ...
  private inferCollection(name: string, type: TokenType): string {
    ...
    // Color collections
    if (type === 'COLOR') {
      if (nameStr.includes('primary') || nameStr.includes('brand')) return 'Brand Colors';
      if (nameStr.includes('semantic') || nameStr.includes('status')) return 'Semantic Colors';
      if (nameStr.includes('neutral') || nameStr.includes('gray')) return 'Neutral Colors';
      return 'Colors';
    }

    // Typography collections  
    if (nameStr.includes('font') || nameStr.includes('text') || nameStr.includes('typography')) {
      return 'Typography';
    }

    // Spacing collections
    if (nameStr.includes('spacing') || nameStr.includes('margin') || nameStr.includes('padding') || nameStr.includes('gap')) {
      return 'Spacing';
    }

    // Size collections
    if (nameStr.includes('size') || nameStr.includes('width') || nameStr.includes('height')) {
      return 'Sizing';
    }
  ...
  private isNumeric(value: string): boolean {
    return /^-?\d*\.?\d+(px|em|rem|%|vh|vw|vmin|vmax|pt|pc|in|cm|mm|ex|ch)?$/.test(value.trim());
  }
