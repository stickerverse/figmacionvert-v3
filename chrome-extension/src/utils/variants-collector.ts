import { VariantData, ElementNode, ComponentRegistry } from '../types/schema';
...
export class VariantsCollector {
  ...
  async collectVariants(
    ...
    this.reportProgress('grouping', 'Grouping variants by element...', 0, entries.length);
    ...
    for (let i = 0; i < entries.length; i++) {
      ...
      const variantSet = this.createVariantSet(element, variants, elementNode, componentRegistry);
      ...
      this.reportProgress('grouping', 'Grouping variants by element...', i + 1, entries.length);
    }
    
    this.reportProgress('optimizing', 'Optimizing variant data...');
    this.optimizeVariants(variantsRegistry);
    ...
    console.log('📊 Variants collection statistics:', variantsRegistry.statistics);
  ...
  private hasSignificantVariants(variants: VariantData[]): boolean {
    ...
    return otherVariants.some(variant => {
      return this.hasSignificantDifferences(defaultVariant.properties, variant.properties);
    });
  }
   ...
   */
  private hasSignificantDifferences(props1: Partial<ElementNode>, props2: Partial<ElementNode>): boolean {
    ...
    if (this.hasDifferentCornerRadius(props1.cornerRadius, props2.cornerRadius)) {
      return true;
    }
  ...
  private hasDifferentFills(fills1?: any[], fills2?: any[]): boolean {
    ...
    return fills1.some((fill1, index) => {
      ...
      if (fill1.color && fill2.color) {
        const colorDiff = Math.abs(fill1.color.r - fill2.color.r) +
  ...
  private hasDifferentStrokes(strokes1?: any[], strokes2?: any[]): boolean {
    ...
    return strokes1.some((stroke1, index) => {
      ...
      if (stroke1.color && stroke2.color) {
        const colorDiff = Math.abs(stroke1.color.r - stroke2.color.r) +
  ...
  private createVariantSet(
    ...
    const statePriority = { default: 0, hover: 1, focus: 2, active: 3, disabled: 4 };
    ...
    return {
      ...
      componentId: elementNode.componentId,
   ...
   */
  private optimizeVariants(registry: VariantsRegistry) {
    ...
    for (const variantSet of variantSets) {
      ...
      const defaultVariant = variantSet.variants.find(v => v.state === 'default');
      if (!defaultVariant) continue;
      
      const otherVariants = variantSet.variants.filter(v => v.state !== 'default');
