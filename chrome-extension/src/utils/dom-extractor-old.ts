...
import { YOUTUBE_CONTENT_CONFIG } from "../config/capture-config";
...
export class DOMExtractor {
  ...
  private isYouTubeHost = false;
  private readonly MAX_EXTRACTION_TIME = 180000; // 180 seconds (increased from 115s for YouTube)
  ...
  private performanceConfig = {
    maxNodesPerCapture: 30000, // Hard cap to avoid infinite DOMs (YouTube/Amazon)
  ...
  >();
  private youtubeElementCache = new WeakMap<Element, boolean>();
  ...
  async extractPageToSchema(): Promise<WebToFigmaSchema> {
    ...
    // CRITICAL: Aggressive navigation prevention for YouTube SPA
    // YouTube uses pushState/replaceState for navigation, so we need to intercept those
    ...
    // CRITICAL FIX: Disable scroll restoration for YouTube before extraction starts
    // This prevents YouTube from restoring scroll position during extraction
    try {
      if (
        window.location.hostname.includes("youtube.com") ||
        window.location.hostname.includes("youtu.be")
        ...
        if ("scrollRestoration" in history) {
          ...
          console.log(
            "🔧 [YOUTUBE] Disabled scroll restoration at extraction start"
          );
        }
      }
    } catch (e) {
    ...
    // Intercept history.pushState and history.replaceState (YouTube's SPA navigation)
      ...
      if (navigationBlocked) {
        if (url) {
          try {
            ...
            // Check for YouTube video ID changes specifically
            ...
            const isChannelNavigation =
              (extractor.isYouTubeChannelPage(originalPath) &&
                !extractor.isYouTubeChannelPage(newPath)) ||
              (!extractor.isYouTubeChannelPage(originalPath) &&
                extractor.isYouTubeChannelPage(newPath)) ||
              (extractor.isYouTubeChannelPage(originalPath) &&
                extractor.isYouTubeChannelPage(newPath) &&
                extractor.extractYouTubeChannelId(originalPath) !==
                  extractor.extractYouTubeChannelId(newPath));
          ...
          // For YouTube, be more lenient with state-only changes (might be needed for player state)
    ...
    // CRITICAL FIX: Block all anchor clicks and form submissions on YouTube
    // YouTube uses anchor tags for navigation, so we need to prevent clicks
    ...
    // Add click blocking for anchors (capture phase to catch before YouTube's handlers)
    ...
    // CRITICAL FIX: Enhanced anchor click blocking for YouTube
    // YouTube uses anchor tags extensively for navigation, so we need aggressive blocking
    const blockNavigationClicks = (e: MouseEvent) => {
      ...
      // ENHANCED: Block ALL navigation clicks, not just YouTube-specific elements
      ...
      if (link) {
        ...
        if (href) {
          try {
            ...
            // Check if it's a different page (different origin, path, or video ID for YouTube)
            const isDifferentPage =
              targetUrlObj.origin !== currentUrlObj.origin ||
              targetUrlObj.pathname !== currentUrlObj.pathname ||
              (targetUrlObj.hostname.includes("youtube.com") &&
      ...
      // Block YouTube-specific navigation elements
      const isYouTubeNavElement =
        target.closest("ytd-video-meta-block") ||
        ...
        target.closest("ytd-video-title") ||
        target.closest("ytd-channel-name") ||
        target.closest('[class*="video"]') ||
        target.closest('[id*="video"]') ||
        target.closest('[role="link"]');

      if (isYouTubeNavElement) {
        console.warn(
          `🚫 [NAVIGATION BLOCK] Blocked click on interactive YouTube element`
        );
        e.preventDefault();
        e.stopPropagation();
        e.stopImmediatePropagation();
    ...
    // ENHANCED: Monitor URL changes VERY frequently (every 5ms for YouTube - extremely aggressive)
    ...
    // ENHANCED: Watch for navigation via hash changes (YouTube sometimes uses this)
          ...
          set: function (value: string | Location) {
            if (navigationBlocked) {
              ...
              if (newUrl && newUrl !== originalUrl) {
                // ENHANCED: Check if it's a different YouTube video (more comprehensive)
    ...
    // ENHANCED: Aggressive scroll reset for dynamic pages like YouTube
    // YouTube pages have scroll restoration and dynamic content that can interfere
    if (currentScrollTop > 1 || currentScrollLeft > 1) {
      ...
      // Multiple scroll attempts for stubborn pages like YouTube
      ...
      while (scrollAttempts < maxAttempts) {
        ...
        // Wait longer for YouTube's dynamic content to settle
        await new Promise<void>((resolve) => {
          // Use multiple RAFs + timeout for YouTube's complex rendering
          requestAnimationFrame(() => {
            requestAnimationFrame(() => {
              requestAnimationFrame(() => {
                // Additional delay for YouTube's scroll restoration
                setTimeout(
                  () => {
                    resolve(undefined);
                  },
        ...
        // If scroll didn't change, we're stuck (YouTube may be preventing scroll)
        if (
          ...
          console.warn(
            `⚠️ [POSITION FIX] Scroll appears stuck (top: ${newScrollTop}, left: ${newScrollLeft}). ` +
              `This may be due to YouTube's scroll restoration. Continuing with current position...`
          );
          // CRITICAL FIX: Disable scroll restoration for YouTube to prevent interference
          try {
            if (
              window.location.hostname.includes("youtube.com") ||
              window.location.hostname.includes("youtu.be")
              ...
              if ("scrollRestoration" in history) {
                (history as any).scrollRestoration = "manual";
                console.log(
                  "🔧 [YOUTUBE] Disabled scroll restoration to prevent interference"
                );
              }
            }
          } catch (e) {
          ...
          // For YouTube, we'll account for scroll offset in positioning calculations
          break;
        }
      ...
      if (finalScrollTop > 1 || finalScrollLeft > 1) {
        ...
        // Store scroll offset for use in positioning calculations
        (this as any).scrollOffset = {
          top: finalScrollTop,
          left: finalScrollLeft,
        };
      } else {
        (this as any).scrollOffset = { top: 0, left: 0 };
      }
    } else {
      (this as any).scrollOffset = { top: 0, left: 0 };
    }

    // ENHANCED: Wait for YouTube dynamic content to load before extraction
    // OPTIMIZED: Reduced wait times and added progress updates to prevent watchdog timeout
    if (
      window.location.hostname.includes("youtube.com") ||
      window.location.hostname.includes("youtu.be")
    ) {
      console.log("⏳ [YOUTUBE] Waiting for dynamic content to load...");
      this.postProgress("Waiting for YouTube content to load...", 25);

      // CRITICAL: Set overall timeout for YouTube content loading
      const contentLoadingStart = Date.now();
      const maxContentLoadTime = YOUTUBE_CONTENT_CONFIG.maxDynamicContentWaitMs;

      // Wait for main video player to be ready (reduced from 1500ms)
      await new Promise((resolve) => setTimeout(resolve, 800));
      this.postProgress("Checking video player...", 27);

      // Check timeout before proceeding
      if (Date.now() - contentLoadingStart > maxContentLoadTime) {
        console.warn(
          "⚠️ [YOUTUBE] Content loading timeout exceeded, proceeding with extraction"
        );
        this.postProgress("Timeout reached, starting extraction...", 33);
      } else {
        // ENHANCED: Find and ensure main video player is loaded
        // Even if page is scrolled, the player should be in DOM
        const mainPlayer = document.querySelector(
          "ytd-watch-flexy, ytd-player, #movie_player, #player-container"
        );
        if (mainPlayer) {
          console.log("✅ [YOUTUBE] Found main video player in DOM");
          // ENHANCED: Use safer method to trigger player initialization without scrollIntoView
          // scrollIntoView can trigger YouTube's SPA navigation, so we use a different approach
          const originalScroll =
            window.pageYOffset || document.documentElement.scrollTop;

          // Instead of scrollIntoView, just scroll to the player's position manually
          // This avoids triggering YouTube's navigation handlers
          try {
            const playerRect = mainPlayer.getBoundingClientRect();
            const targetScroll =
              originalScroll + playerRect.top - window.innerHeight / 2;
            window.scrollTo({
              top: Math.max(0, targetScroll),
              behavior: "instant",
            });
            await new Promise((resolve) => setTimeout(resolve, 200)); // Reduced wait
            this.postProgress("Video player ready...", 28);
          } catch (scrollError) {
            console.warn(
              "⚠️ [YOUTUBE] Could not scroll to player:",
              scrollError
            );
          }

          // Restore scroll position (or try to)
          try {
            window.scrollTo({ top: originalScroll, behavior: "instant" });
            await new Promise((resolve) => setTimeout(resolve, 100));
          } catch (restoreError) {
            console.warn(
              "⚠️ [YOUTUBE] Could not restore scroll:",
              restoreError
            );
          }
        }

        // Check timeout before comments loading
        if (
          Date.now() - contentLoadingStart >
          YOUTUBE_CONTENT_CONFIG.skipCommentsAfterMs
        ) {
          console.warn(
            "⚠️ [YOUTUBE] Skipping comments loading due to time limit"
          );
          this.postProgress("Skipping comments, starting extraction...", 33);
        } else {
          // ENHANCED: Find comments section and ensure it's loaded
          // Comments might be lazy-loaded, so we need to trigger them
          this.postProgress("Checking comments section...", 29);
          const commentsSection = document.querySelector(
            "ytd-comments, ytd-comment-thread-renderer, #comments"
          );
          if (commentsSection) {
            console.log("✅ [YOUTUBE] Found comments section in DOM");
            // ENHANCED: Use safer method to trigger comments loading without scrollIntoView
            // scrollIntoView can trigger YouTube's SPA navigation
            const originalScroll =
              window.pageYOffset || document.documentElement.scrollTop;

            // Instead of scrollIntoView, scroll to comments position manually
            try {
              const commentsRect = commentsSection.getBoundingClientRect();
              const targetScroll = originalScroll + commentsRect.top - 100; // 100px from top
              window.scrollTo({
                top: Math.max(0, targetScroll),
                behavior: "instant",
              });

              // CRITICAL: Timeout check for comments loading
              const commentsStartTime = Date.now();
              await new Promise((resolve) => setTimeout(resolve, 500)); // Wait for lazy load

              if (
                Date.now() - commentsStartTime >
                YOUTUBE_CONTENT_CONFIG.contentLoadRetryMs * 2
              ) {
                console.warn(
                  "⚠️ [YOUTUBE] Comments loading taking too long, skipping"
                );
              } else {
                this.postProgress("Loading comments...", 30);

                // Try to load more comments by scrolling within the section
                const commentThreads = commentsSection.querySelectorAll(
                  "ytd-comment-thread-renderer"
                );
                if (commentThreads.length === 0) {
                  console.log(
                    "⏳ [YOUTUBE] No comment threads found, waiting longer..."
                  );
                  ...
                  await new Promise((resolve) =>
                    setTimeout(
                      resolve,
                      YOUTUBE_CONTENT_CONFIG.contentLoadRetryMs
                    )
                  );
                }
              }
              ...
              console.warn(
                "⚠️ [YOUTUBE] Could not scroll to comments:",
                scrollError
              );
            }
            ...
            console.warn("⚠️ [YOUTUBE] Comments section not found in DOM");
          }
        }

        // Initialize page height for final cleanup
        ...
        // Check timeout before page scrolling
        if (Date.now() - contentLoadingStart > maxContentLoadTime) {
          console.warn(
            "⚠️ [YOUTUBE] Content loading timeout exceeded, proceeding with extraction"
          );
          ...
          // ENHANCED: Scroll through entire page to trigger lazy-loading of all content
          // This ensures we capture related videos, description, and all comments
          this.postProgress("Loading all page content...", 31);
          console.log(
            "⏳ [YOUTUBE] Scrolling through page to load all content..."
          );

          const originalScroll =
            window.pageYOffset || document.documentElement.scrollTop;

          // CRITICAL FIX: Limited scrolling with timeout protection
          // Reduce scroll steps to prevent timeout but still load key content
          const scrollSteps = Math.min(
            YOUTUBE_CONTENT_CONFIG.maxScrollAttempts,
            6
          );
          let stepHeight = pageHeight / scrollSteps;

          for (let step = 1; step <= scrollSteps; step++) {
            // Check timeout on each step
            if (Date.now() - contentLoadingStart > maxContentLoadTime) {
              console.warn(
                `⚠️ [YOUTUBE] Timeout during scroll step ${step}, stopping`
              );
              break;
            }

            const targetScroll = Math.min(step * stepHeight, pageHeight);
            window.scrollTo({ top: targetScroll, behavior: "instant" });
            // OPTIMIZED: Reduced wait time to prevent timeout
            await new Promise((resolve) =>
              setTimeout(resolve, YOUTUBE_CONTENT_CONFIG.contentLoadRetryMs)
            );

            // CRITICAL: Trigger any intersection observers or lazy-load handlers
            // Dispatch a scroll event to ensure YouTube's lazy-loading triggers
            try {
            ...
            // Check if more content loaded (page height might have increased)
            const newPageHeight = Math.max(
              document.documentElement.scrollHeight,
              document.body.scrollHeight
            );
            if (newPageHeight > pageHeight) {
              // Content loaded, adjust remaining steps
              const remainingSteps = scrollSteps - step;
              const remainingHeight = newPageHeight - targetScroll;
              if (remainingHeight > 0 && remainingSteps > 0) {
                stepHeight = remainingHeight / remainingSteps;
                pageHeight = newPageHeight; // Update page height
              }
            }
          }
        }
        ...
        if (Date.now() - contentLoadingStart < maxContentLoadTime) {
          // Scroll to very bottom to ensure all content is loaded
          window.scrollTo({ top: pageHeight, behavior: "instant" });
          await new Promise((resolve) => setTimeout(resolve, 300)); // Reduced wait

          // ENHANCED: Trigger one more scroll event to catch any final lazy-loading
          try {
            window.dispatchEvent(new Event("scroll"));
            await new Promise((resolve) => setTimeout(resolve, 200)); // Reduced wait
          } catch (e) {
          ...
          // Scroll back to top for extraction (but content is now loaded in DOM)
          window.scrollTo({ top: 0, behavior: "instant" });
          await new Promise((resolve) => setTimeout(resolve, 200)); // Reduced wait
        } else {
          // Timeout reached, just scroll to top
          console.warn("⚠️ [YOUTUBE] Skipping final cleanup due to timeout");
          window.scrollTo({ top: 0, behavior: "instant" });
        }

        console.log("✅ [YOUTUBE] All page content loaded");
        this.postProgress("All content loaded, starting extraction...", 32);

        console.log("✅ [YOUTUBE] Dynamic content wait complete");
        this.postProgress("YouTube content ready, starting extraction...", 33);
      } // Close the first timeout check if block
    }
    ...
    try {
          ...
          if (isYT) {
            fetch(
              "http://127.0.0.1:7242/ingest/ec6ff4c5-673b-403d-a943-70cb2e5565f2",
              {
                ...
                body: JSON.stringify({
                  location: "dom-extractor.ts:extractPageToSchema:root",
                  message: "YouTube root extracted",
                  data: {
                    url: window.location.href,
                    rootId: rootNode.id,
                    ...
                    rootChildren:
                      Array.isArray((rootNode as any).children) &&
                      (rootNode as any).children.length >= 0
                        ? (rootNode as any).children.length
                        : -1,
                    nodeIdCounter: this.nodeId,
                  },
                  timestamp: Date.now(),
                  sessionId: "debug-session",
                  runId: "run1",
                  hypothesisId: "H_YT_EMPTY_TREE",
                }),
              }
            ).catch(() => {});
          }
            ...
            if (isYT) {
              fetch(
                "http://127.0.0.1:7242/ingest/ec6ff4c5-673b-403d-a943-70cb2e5565f2",
                {
                  ...
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({
                    location: "dom-extractor.ts:extractPageToSchema",
                    message: "Cleared root body/html backgrounds",
                    data: {
                      url: window.location.href,
                      htmlTag: rootNode.htmlTag,
                      beforeFills,
                      afterFills: Array.isArray((rootNode as any).fills)
                        ? (rootNode as any).fills.length
                        : 0,
                      beforeBackgrounds: beforeBgs,
                      afterBackgrounds: Array.isArray(
                        (rootNode as any).backgrounds
                      )
                        ? (rootNode as any).backgrounds.length
                        : 0,
                      styleBg: (rootNode as any)?.style?.backgroundColor,
                      computedBg: (rootNode as any)?.computedStyle
                        ?.backgroundColor,
                    },
                    timestamp: Date.now(),
                    sessionId: "debug-session",
                    runId: "run1",
                    hypothesisId: "H1_ROOT_BG_CLEAR",
                  }),
                }
              ).catch(() => {});
            }
      ...
      try {
        const isYT =
          ...
          window.location.hostname.includes("youtu.be");
        if (isYT) {
          const imgCount = this.assets.images.size;
          const errs = this.errorTracker.getSummary();
          const rootChildren = Array.isArray((schema.root as any)?.children)
            ? (schema.root as any).children.length
            : -1;
          fetch(
            "http://127.0.0.1:7242/ingest/ec6ff4c5-673b-403d-a943-70cb2e5565f2",
            {
              ...
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
                location: "dom-extractor.ts:extractPageToSchema:final",
                message: "YouTube schema finalize",
                data: {
                  url: window.location.href,
      ...
      // CRITICAL FIX: Skip on YouTube pages to avoid timeout/errors (YouTube has too many buttons)
      const isYouTube =
        window.location.hostname.includes("youtube.com") ||
        window.location.hostname.includes("youtu.be");
      if (!isYouTube) {
        ...
        console.log(
          "ℹ️ [HOVER] Skipping hover state capture on YouTube (too many buttons)"
        );
      }
  ...
  private async extractNode(
    ...
    // ENHANCED: Use stored scroll offset if available (from scroll reset attempt)
    // This accounts for cases where scroll couldn't be fully reset (e.g., YouTube)
    const scrollOffset = (this as any).scrollOffset || { top: 0, left: 0 };
    const currentScrollTop =
      window.pageYOffset || document.documentElement.scrollTop;
    const currentScrollLeft =
      window.pageXOffset || document.documentElement.scrollLeft;

    // Use the stored offset if available, otherwise use current scroll
    // This ensures consistency throughout extraction
    const scrollTop = ExtractionValidation.safeParseFloat(
      scrollOffset.top > 0 ? scrollOffset.top : currentScrollTop,
      0
    );
    const scrollLeft = ExtractionValidation.safeParseFloat(
      scrollOffset.left > 0 ? scrollOffset.left : currentScrollLeft,
      0
    );
    ...
    // ENHANCED: Zero-size check with exceptions for YouTube elements
    // YouTube video players and comments might be off-screen but still important
    const elementTagName = element.tagName.toLowerCase();
    const isYouTubeImportant =
      this.isYouTubeElement(element) ||
      elementTagName.startsWith("ytd-") ||
    ...
    if (rect.width === 0 && rect.height === 0 && element.tagName !== "BODY") {
      ...
      // ENHANCED: Don't skip YouTube important elements even if zero-size
      ...
      // For YouTube important elements, try to get dimensions from computed styles
      if (isYouTubeImportant && (rect.width === 0 || rect.height === 0)) {
    ...
    if (isFixedOrSticky) {
      ...
      absoluteX = Math.round(rectLeft);
      absoluteY = Math.round(rectTop);
    } else {
      // Regular elements use document coordinates (add scroll offset for positioning)
      absoluteX = Math.round(rectLeft + validScrollLeft);
      absoluteY = Math.round(rectTop + validScrollTop);
    }
    ...
    const borderTop = ExtractionValidation.safeParseFloat(computed.borderTopWidth, 0);
    const borderRight = ExtractionValidation.safeParseFloat(computed.borderRightWidth, 0);
    const borderBottom = ExtractionValidation.safeParseFloat(computed.borderBottomWidth, 0);
    const borderLeft = ExtractionValidation.safeParseFloat(computed.borderLeftWidth, 0);
    
    const paddingTop = ExtractionValidation.safeParseFloat(computed.paddingTop, 0);
    const paddingRight = ExtractionValidation.safeParseFloat(computed.paddingRight, 0);
    const paddingBottom = ExtractionValidation.safeParseFloat(computed.paddingBottom, 0);
    const paddingLeft = ExtractionValidation.safeParseFloat(computed.paddingLeft, 0);

    // CRITICAL: Calculate content dimensions for Figma
    // getBoundingClientRect() always gives border-box dimensions
    // For Figma, we need to separate content area from borders/padding
    ...
    // Calculate content area dimensions (what Figma expects)
    const contentWidth = Math.max(0, dimensions.width - totalHorizontalBorder - totalHorizontalPadding);
    const contentHeight = Math.max(0, dimensions.height - totalVerticalBorder - totalVerticalPadding);
    
    // Store both border-box (visual) and content-box (Figma) dimensions
    const visualDimensions = { width: dimensions.width, height: dimensions.height };
    const contentDimensions = { width: contentWidth, height: contentHeight };
    
    // Use visual dimensions for layout (this maintains pixel-perfect positioning)
    // but store content dimensions for Figma node creation when needed
    const figmaDimensions = boxSizing === "border-box" ? contentDimensions : visualDimensions;
    ...
    const absoluteLayout = {
      ...
      right: validatedAbsoluteX + dimensions.width, // Now guaranteed to be valid
      bottom: validatedAbsoluteY + dimensions.height, // Now guaranteed to be valid
      width: dimensions.width, // Validated above
      height: dimensions.height, // Validated above
    };
    ...
    // ENHANCED: For YouTube video player containers, ensure thumbnail is applied
    ...
    const isMainPlayer =
      ...
      (tagName.includes("player") ||
        tagName.startsWith("ytd-player") ||
        tagName.startsWith("ytd-watch") ||
        element.id === "movie_player" ||
        element.id === "player-container" ||
        element.classList.contains("ytd-player") ||
    ...
    if (isMainPlayer) {
      ...
      if (urlMatch && urlMatch[1]) {
        ...
        const hasThumbnail = node.fills?.some(
          (f: any) => f.type === "IMAGE" && f.url?.includes("youtube.com/vi")
        );
          ...
          for (const thumbnailUrl of thumbnailOptions) {
            try {
              ...
              if (asset) {
                ...
                node.imageHash = key;
                console.log(
                  `✅ [YOUTUBE] Applied thumbnail to player container for video ${videoId}`
                );
    ...
    // ENHANCED: Add YouTube-specific metadata for better rendering
    if (this.isYouTubeElement(element)) {
      node.isYouTubeElement = true;
      // Store additional context for YouTube elements
      if (!node.metadata) node.metadata = {};
      node.metadata.youtubeContext = {
  ...
  /**
   * CRITICAL: YouTube theme background enforcement to fix white hole regression.
   * When structural containers have transparent backgrounds but theme variables
   * indicate a theme background, apply the theme color.
   */
  private async applyYouTubeThemeBackgroundEnforcement(
    computed: CSSStyleDeclaration,
    element: Element,
    node: any
  ): Promise<void> {
    try {
      ...
      // Only apply if current background is transparent or missing
      ...
      if (
        ...
        return; // Already has a background
      }

      // Check if this is a key structural container
      const isStructuralContainer = this.isYouTubeStructuralContainer(element);
      if (!isStructuralContainer) {
        return;
      }

      // Get theme background from CSS variables
      const themeBackground = this.getYouTubeThemeBackground();
      if (!themeBackground) {
        return;
      }

      // Parse and apply the theme background
      const color = this.parseColorSafe(themeBackground);
      if (color && color.a > 0) {
        if (!node.fills) node.fills = [];

        // Add theme background as the bottom layer
        node.fills.unshift({
          type: "SOLID",
          color: { r: color.r, g: color.g, b: color.b, a: color.a },
          opacity: color.a,
          visible: true,
          // Mark as theme enforcement to distinguish from regular backgrounds
          themeEnforcement: true,
        });

        console.log(
          `✅ [YOUTUBE] Applied theme background to ${element.tagName}#${
            element.id || ""
          }.${this.getClassNameSafe(element) || ""}: ${themeBackground}`
        );
      }
    } catch (error) {
      console.warn("YouTube theme background enforcement error:", error);
    }
  }

  /**
   * Check if element is a YouTube structural container that needs theme background
   */
  private isYouTubeStructuralContainer(element: Element): boolean {
    const tagName = element.tagName.toLowerCase();
    const className = this.getClassNameSafe(element);
    const id = String(element.id || "");

    // Evidence-based selectors for known structural containers
    ...
    // Check tag names first (most reliable)
    ...
    // Check for generic structural containers in YouTube
    if (
      (tagName === "div" ||
        tagName === "section" ||
        tagName === "main" ||
        tagName === "html" ||
        tagName === "body") &&
      (className.includes("style-scope") || className.includes("container"))
    ) {
      return true;
    }

    // Also catch root elements directly on YouTube
  ...
  /**
   * Get YouTube theme background color from CSS variables
   */
  private getYouTubeThemeBackground(): string | null {
    try {
      const root = document.documentElement;
      const computedRoot = getComputedStyle(root);

      // YouTube theme CSS variables (in priority order)
      const themeVariables = [
        ...
        "--yt-main-app-background-color",
      ];

      for (const variable of themeVariables) {
        const value = computedRoot.getPropertyValue(variable);
        if (value && value.trim() && value !== "transparent") {
          return value.trim();
        }
      }

      // Fallback: detect from document background
      ...
      if (htmlBg && htmlBg !== "transparent") {
        return htmlBg;
      }

      return null;
    } catch (error) {
      console.warn("Error getting YouTube theme background:", error);
      return null;
    }
  }
  ...
  // ENHANCED: Detect if an element is a YouTube-specific element that needs special handling
  private isYouTubeElement(element: Element): boolean {
    if (!this.isYouTubeHost) return false;

    if (this.youtubeElementCache.has(element)) {
      return this.youtubeElementCache.get(element)!;
    }
    ...
    try {
      ...
      // Check for YouTube web component tags FIRST (ytd-*, yt-*)
      if (
        ...
        if (
          ...
          // Check class names (safely handling SVGAnimatedString)
          const classStr = this.getClassNameSafe(element);

          if (
            classStr.includes("yt-") ||
            classStr.includes("youtube") ||
            classStr.includes("ytd-") ||
            classStr.includes("video-player") ||
            classStr.includes("watch-content")
          ) {
            result = true;
          }
        }
      ...
      // Ancestor check is expensive, only do it if still not identified and we are on YT
      if (!result) {
        let parent = element.parentElement;
        let depth = 0;
        while (parent && depth < 3) {
          // Reduced depth for performance
          const parentTag = parent.tagName.toLowerCase();
          if (parentTag.startsWith("ytd-") || parentTag.startsWith("yt-")) {
            result = true;
            break;
          }
      ...
      // Ignore errors in element property access
    }

    this.youtubeElementCache.set(element, result);
    return result;
  }
  ...
  private extractTextContentSafe(element: Element): string | null {
    try {
      // ENHANCED: Special handling for YouTube comments to extract actual content
      const tagName = element.tagName.toLowerCase();
      if (tagName.startsWith("ytd-") && tagName.includes("comment")) {
        // For YouTube comment elements, use innerText to get rendered content
  ...
  private async processChildrenSafe(
    ...
    try {
      ...
      // YOUTUBE CHILD CAP: prevent infinite/huge subtrees (comments, rich grids) from stalling extraction
      // This is not a partial schema; it defines deterministic bounds for unbounded DOM regions.
      let boundedChildNodes: ChildNode[] = childNodes;
      if (this.isYouTubeHost) {
        const tag = element.tagName.toLowerCase();
        const id = (element as HTMLElement).id
          ? (element as HTMLElement).id.toLowerCase()
          : "";
        const isCommentsTree =
          id === "comments" ||
          tag === "ytd-comments" ||
        ...
        if (isCommentsTree && childNodes.length > 50) {
          // Aggressive limit for comments (user feedback: shouldn't take long)
          boundedChildNodes = childNodes.slice(0, 50);
        } else if (isRecommendationList && childNodes.length > 30) {
          // Aggressive limit for recommendations
          boundedChildNodes = childNodes.slice(0, 30);
        } else if (childNodes.length > 2000) {
          // Lowered generic safeguard from 5000 to 2000
          boundedChildNodes = childNodes.slice(0, 2000);
        }
      }
  ...
  /**
   * YOUTUBE SPECIFIC: Enforce theme background for main containers
   * This is critical to prevent "white hole" bugs where transparent containers
   * fail to inherit the dark mode background.
   */
  private getYouTubeThemeColor(element: Element): string | null {
    try {
      const isYouTube =
        window.location.hostname.includes("youtube.com") ||
        window.location.hostname.includes("youtu.be");
      ...
      // Try YouTube specific CSS variables for background
      // --yt-spec-base-background is the main one (usually #0f0f0f in dark mode)
      const baseBg = computed.getPropertyValue("--yt-spec-base-background");
      if (baseBg && baseBg.trim()) {
        return baseBg.trim();
      }

      // Fallback variables
      const generalBg = computed.getPropertyValue(
        "--yt-spec-general-background-a"
      );
      if (generalBg && generalBg.trim()) {
        return generalBg.trim();
      }

      const brandBg = computed.getPropertyValue(
        "--yt-spec-brand-background-primary"
      );
      if (brandBg && brandBg.trim()) {
        return brandBg.trim();
      }
  ...
  private getYouTubeEnforcedBackground(element: Element): string | null {
    try {
      const isYouTube =
        window.location.hostname.includes("youtube.com") ||
        window.location.hostname.includes("youtu.be");
      ...
      const tagName = element.tagName.toLowerCase();
      const id = element.id;
      // List of containers that are known to "be" the page background
      if (
        tagName === "ytd-app" ||
        tagName === "ytd-watch-flexy" ||
        ...
        return this.getYouTubeThemeColor(element);
      }
      return null;
    } catch {
      return null;
  ...
  private async extractStylesSafe(
    ...
    try {
      ...
      if (bgColor) {
        ...
        // YOUTUBE SPECIFIC: Check for enforced theme color on specific transparent containers
        if (
          (!effectiveColorParsed || effectiveColorParsed.a < 0.01) &&
          (window.location.hostname.includes("youtube.com") ||
            window.location.hostname.includes("youtu.be"))
        ) {
          const enforced = this.getYouTubeEnforcedBackground(element);
          if (enforced) {
            effectiveBgColor = enforced;
            effectiveColorParsed = this.parseColorSafe(enforced);
            console.log(
              `📺 [YOUTUBE] Enforced theme background for ${element.tagName}`
            );
          }
        }
      ...
      // CRITICAL: YouTube theme background enforcement to fix white hole regression
      await this.applyYouTubeThemeBackgroundEnforcement(
        computed,
        element,
        node
      );
      ...
      try {
        ...
        if (isYT) {
          ...
          const fillsCount = Array.isArray(node?.fills) ? node.fills.length : 0;
          const isDocRoot =
          ...
          const shouldLog =
            (!isDocRoot &&
              ...
              bg !== "rgba(0, 0, 0, 0)") ||
            (bgImg && bgImg !== "none");
          const limitKey = "__DBG_YT_STYLE__";
          const used = (globalThis as any)[limitKey] || 0;
          if (shouldLog && used < 20) {
            (globalThis as any)[limitKey] = used + 1;
            fetch(
              "http://127.0.0.1:7242/ingest/ec6ff4c5-673b-403d-a943-70cb2e5565f2",
              {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                  location: "dom-extractor.ts:extractStylesSafe",
                  message: "YouTube style extraction (background/fills)",
                  data: {
                    tag: element.tagName,
                    id: (element as HTMLElement).id || "",
                    className:
                      typeof (element as any).className === "string"
                        ? (element as any).className.slice(0, 120)
                        : "",
                    isDocRoot,
                    computedBg: bg,
                    computedBgImg: bgImg.slice(0, 200),
                    fillsCount,
                    storedComputedBg: node?.computedStyle?.backgroundColor,
                    storedStyleBg: node?.style?.backgroundColor,
                  },
                  timestamp: Date.now(),
                  sessionId: "debug-session",
                  runId: "run1",
                  hypothesisId: "H2_BG_PARSE_OR_SKIP",
                }),
              }
            ).catch(() => {});
          }
      ...
      const paddingTop = node._boxSizingData?.paddings.top ?? ExtractionValidation.safeParseFloat(computed.paddingTop, 0);
      const paddingRight = node._boxSizingData?.paddings.right ?? ExtractionValidation.safeParseFloat(computed.paddingRight, 0);
      const paddingBottom = node._boxSizingData?.paddings.bottom ?? ExtractionValidation.safeParseFloat(computed.paddingBottom, 0);
      const paddingLeft = node._boxSizingData?.paddings.left ?? ExtractionValidation.safeParseFloat(computed.paddingLeft, 0);
  ...
  private async extractSpecialPropertiesSafe(
    ...
    try {
      // ENHANCED: Handle YouTube custom web components before standard tags
      // CRITICAL FIX: Check for ALL YouTube web component patterns (ytd-*, yt-*)
      if (
        tagName.startsWith("ytd-") ||
        tagName.startsWith("yt-") ||
        this.isYouTubeElement(element)
      ) {
        await this.handleYouTubeWebComponent(element, node, computed);
  ...
  private async inlineSvgUsesInPage(
    ...
    try {
      ...
      // outerHTML can trigger Trusted Types violations in CSP-protected pages (YouTube, etc.)
    ...
    if (imageUrl && ExtractionValidation.isValidUrl(imageUrl)) {
      ...
      node.fills = [
        {
          ...
          scaleMode: scaleMode,
          visible: true,
          // CRITICAL: Store URL in fill for fallback when base64 is missing (lazy-loaded images)
          url: imageUrl,
        },
      ];
      node.imageHash = key;
      ...
      try {
        const isYT =
          window.location.hostname.includes("youtube.com") ||
          window.location.hostname.includes("youtu.be");
        const usedKey = "__DBG_YT_IMG__";
        const used = (globalThis as any)[usedKey] || 0;
        if (isYT && used < 30) {
          (globalThis as any)[usedKey] = used + 1;
          fetch(
            "http://127.0.0.1:7242/ingest/ec6ff4c5-673b-403d-a943-70cb2e5565f2",
            {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
                location: "dom-extractor.ts:handleImageElement",
                message: "YouTube <img> node emitted",
                data: {
                  url: imageUrl,
                  imageHash: key,
                  ...
                  naturalWidth,
                  naturalHeight,
                  nodeW: node?.absoluteLayout?.width ?? node?.layout?.width,
                  nodeH: node?.absoluteLayout?.height ?? node?.layout?.height,
                },
                timestamp: Date.now(),
                sessionId: "debug-session",
                runId: "run1",
                hypothesisId: "H3_IMG_ASPECT_SCHEMA",
              }),
            }
          ).catch(() => {});
        }
  ...
  private async handleEmbedElement(element: Element, node: any): Promise<void> {
    ...
    if (tagName === "iframe") {
      ...
      if (embedSrc && /youtube\.com|youtu\.be/.test(embedSrc)) {
        embedType = "youtube";
        node.name = "YouTube Video";
      } else {
        embedType = "iframe";
    ...
    // ENHANCED: For YouTube iframes, extract thumbnail with multiple fallback options
    if (embedType === "youtube" && embedSrc) {
      try {
        // ENHANCED: Extract video ID from all YouTube URL patterns
        ...
        if (videoId) {
          ...
          for (const thumbnailUrl of thumbnailOptions) {
            try {
              ...
              if (asset) {
                ...
                console.log(
                  `✅ [YOUTUBE] Captured thumbnail for video ${videoId}`
                );
          ...
          if (!thumbnailCaptured) {
            console.warn(
              `⚠️ [YOUTUBE] Could not capture thumbnail for video ${videoId}`
            );
          ...
          // ENHANCED: Store comprehensive YouTube metadata
          node.embed = {
            ...node.embed,
            videoId: videoId,
            platform: "youtube",
          ...
          console.warn(
            `⚠️ [YOUTUBE] Could not extract video ID from URL: ${embedSrc}`
          );
        }
      } catch (error) {
        // YouTube parsing failed, continue without thumbnail
        this.errorTracker.recordError(
          "handleEmbedElement",
          `Failed to extract YouTube thumbnail: ${
  ...
  // ENHANCED: Handle YouTube custom web components (ytd-player, ytd-watch-flexy, ytd-comments, etc.)
  private async handleYouTubeWebComponent(
    element: Element,
    node: any,
    computed: CSSStyleDeclaration
  ): Promise<void> {
    try {
      ...
      // YouTube video player component (ytd-player, ytd-watch-flexy, #movie_player)
      if (
        tagName === "ytd-player" ||
        tagName === "ytd-watch-flexy" ||
        tagName.includes("player") ||
        ...
        node.name = node.name || "YouTube Video Player";
        ...
        if (urlMatch && urlMatch[1]) {
          videoId = urlMatch[1];
          console.log(`✅ [YOUTUBE] Extracted video ID from URL: ${videoId}`);
        }
        ...
        if (!videoId) {
          ...
          if (iframe) {
            ...
            if (iframeMatch && iframeMatch[1]) {
              ...
              console.log(
                `✅ [YOUTUBE] Extracted video ID from iframe: ${videoId}`
              );
            }
          }
        }
        ...
        if (!videoId) {
          ...
          if (videoIdAttr) {
            videoId = videoIdAttr;
            console.log(
              `✅ [YOUTUBE] Extracted video ID from attribute: ${videoId}`
            );
          }
        }
        ...
        if (videoId) {
          ...
          for (const thumbUrl of thumbnailOptions) {
            try {
              ...
              if (asset) {
                ...
                thumbnailCaptured = true;
                console.log(
                  `✅ [YOUTUBE] Captured thumbnail for main player video ${videoId} (${
          ...
          if (thumbnailCaptured && thumbnailKey && thumbnailUrl) {
            ...
            console.log(
              `✅ [YOUTUBE] Applied thumbnail fill to main player node`
            );
          }

          if (!thumbnailCaptured) {
            console.warn(
              `⚠️ [YOUTUBE] Could not capture thumbnail for main player video ${videoId}`
            );
            node.videoMetadata = {
              platform: "youtube",
              videoId: videoId,
      ...
      // YouTube comments section (ytd-comments, ytd-comment-thread-renderer)
      if (
        tagName === "ytd-comments" ||
        tagName === "ytd-comment-thread-renderer" ||
        ...
        node.name = node.name || "YouTube Comments";
        node.isYouTubeComments = true;

        // ENHANCED: Wait for comments to load and extract actual comment content
        try {
          // Check if comments are already loaded
          let commentElements = element.querySelectorAll(
          ...
          if (commentElements.length === 0) {
            ...
            // ENHANCED: Use safer scroll method to avoid triggering YouTube navigation
            const originalScroll =
              window.pageYOffset || document.documentElement.scrollTop;
            ...
            try {
              ...
              commentElements = element.querySelectorAll(
                "ytd-comment-thread-renderer, ytd-comment-renderer, ytd-comment-view-model, ytd-comment-simplebox-renderer"
              );
              ...
              if (commentElements.length === 0) {
                ...
                commentElements = element.querySelectorAll(
                  "ytd-comment-thread-renderer, ytd-comment-renderer, ytd-comment-view-model, ytd-comment-simplebox-renderer, #comment, [id*='comment'], [class*='comment']"
                );
              }
            } catch (scrollError) {
              console.warn(
                "⚠️ [YOUTUBE] Could not scroll to comments element:",
                scrollError
              );
            }
          ...
          if (commentElements.length > 0) {
            console.log(
              `✅ [YOUTUBE] Found ${commentElements.length} comment threads`
            );
            ...
            if (commentsDisabled || commentsHidden) {
              console.log(
                "ℹ️ [YOUTUBE] Comments appear to be disabled or hidden on this video"
              );
              node.commentsDisabled = true;
            } else {
              // Only log as info - this is expected for some videos (comments disabled, age-restricted, etc.)
              console.log(
                "ℹ️ [YOUTUBE] Comments section found but no comment threads loaded (may be disabled, age-restricted, or still loading)"
              );
            }
          }
          ...
          if (commentCountEl) {
            ...
            console.log(`✅ [YOUTUBE] Comment count: ${commentCount}`);
          }
        } catch (error) {
          console.warn(
            `⚠️ [YOUTUBE] Error handling comments: ${
              error instanceof Error ? error.message : "Unknown"
            }`
          );
        }
      ...
      // Mark as YouTube element for special handling
      node.isYouTubeElement = true;
      if (!node.metadata) node.metadata = {};
      node.metadata.youtubeComponent = {
        tagName: tagName,
        className: this.getClassNameSafe(element),
        id: element.id || "",
      };
  ...
  /**
   * Checks if a YouTube pathname is a channel page
   */
  private isYouTubeChannelPage(pathname: string): boolean {
    return (
      pathname.startsWith("/@") ||
  ...
  /**
   * Extracts YouTube channel identifier from pathname
   * Returns channel ID, @handle, or null
   */
  private extractYouTubeChannelId(pathname: string): string | null {
  ...
  private validateBoxSizingDimensions(
    ...
    const { boxSizing, visualDimensions, contentDimensions, borders, paddings } = boxSizingData;
    
    // Check for negative content dimensions (indicates calculation error)
    if (contentDimensions.width < 0 || contentDimensions.height < 0) {
      console.warn(`⚠️ [BOX-SIZING] Negative content dimensions detected:`, {
    ...
    const totalBorderPadding = borders.top + borders.right + borders.bottom + borders.left +
                               paddings.top + paddings.right + paddings.bottom + paddings.left;
    const dimensionSum = visualDimensions.width + visualDimensions.height;
    
    if (totalBorderPadding > dimensionSum * 0.8) {
      console.warn(`⚠️ [BOX-SIZING] Border/padding exceeds 80% of element dimensions:`, {
        element: element.tagName,
        totalBorderPadding,
        dimensionSum,
        ratio: totalBorderPadding / dimensionSum
      });
    }

    // Validate box-sizing consistency
    if (boxSizing === "border-box" && contentDimensions.width === visualDimensions.width) {
      console.warn(`⚠️ [BOX-SIZING] border-box element has identical content and visual dimensions - possible extraction error`);
    }
  }
