...
export class LayoutValidator {
  ...
  async validate(
    ...
    const avgPositionAccuracy =
      this.totalNodes > 0
        ? Math.sqrt(
            (this.accuracyMetrics.totalDeltaX ** 2 +
              this.accuracyMetrics.totalDeltaY ** 2) /
              this.totalNodes
          )
        : 0;
    ...
    return {
      ...
      accuracyMetrics: {
        ...
        coordinateSystemsUsed: Array.from(
          this.accuracyMetrics.coordinateSystemsUsed
        ),
      },
  ...
  private async validateNode(
    ...
    // Yield every 100 nodes to prevent main thread blocking
    if (this.totalNodes % 100 === 0) {
      await new Promise((resolve) => setTimeout(resolve, 0));
      if (progressCallback) {
        progressCallback(
          `Validating layout structure... (${this.totalNodes} nodes)`
        );
      }
    }
    ...
    if (x < 0 || y < 0) {
      ...
      this.issues.push({
        ...
        suggestion:
          "Negative positions are valid but may indicate overflow content",
      });
    }
    ...
    if (
      absoluteX > this.viewportWidth * 2 ||
      absoluteY > this.viewportHeight * 2
    ) {
  ...
  private validateGridLayout(node: ElementNode) {
    ...
    if (!gridLayout.templateColumns || !gridLayout.templateRows) {
      this.issues.push({
        ...
        suggestion:
          "Grid will be stored as metadata but rendered as Auto Layout in Figma",
      });
    }
  }
   ...
   * Check for overlapping sibling nodes (potential layout issues)
   */
  private async checkOverlappingSiblings(
    node: ElementNode,
    progressCallback?: (message: string) => void,
    startTime = Date.now()
  ) {
    // Abort if we've spent more than 5 seconds checking overlaps
    ...
    // Check for overlaps among siblings
    // Limit total comparisons to prevent stalls on huge lists
    const MAX_COMPARISONS = 10000;
    let comparisons = 0;

    if (node.children.length > 500) {
      // Optimization for large lists: only check adjacent siblings and a small window
      for (let i = 0; i < node.children.length - 1; i++) {
        // Yield occasionally
        if (i % 200 === 0) {
          await new Promise((resolve) => setTimeout(resolve, 0));
          if (Date.now() - startTime > 5000) return; // Time check
        }

        const child1 = node.children[i];
        const child2 = node.children[i + 1];
        this.checkPairOverlap(child1, child2);
      }
    } else {
      // O(N^2) check for smaller lists, but with comparison limit
  ...
  private checkPairOverlap(childA: ElementNode, childB: ElementNode) {
    if (!childA.layout || !childB.layout) return;
    ...
    if (overlapAmount > 0) {
      ...
      if (overlapAmount > smallerArea * 0.1) {
        this.issues.push({
          ...
          suggestion:
            "Overlapping nodes may indicate z-index layering or absolute positioning",
        });
      }
    }
  }
   ...
   */
  private validateCoordinateAccuracy(
    node: ElementNode
    ...
    // STRICT CHECK: Use hard thresholds instead of confidence scores
    const isAccurate =
      deltaX <= this.thresholds.positionTolerance &&
      deltaY <= this.thresholds.positionTolerance;

    // Confidence is now binary-ish: 1.0 if accurate, 0.0 if not (for backward compatibility)
    const confidence = isAccurate ? 1.0 : 0.0;
    ...
    this.accuracyMetrics.worstDelta = Math.max(
      this.accuracyMetrics.worstDelta,
    ...
    if (!isAccurate) {
      ...
      this.issues.push({
        severity: "error", // Upgraded from warning/info to error
        type: "coordinate-accuracy",
        nodeId: node.id,
        nodeName: node.name,
        message: `Coordinate mismatch: ${deltaX.toFixed(2)}px, ${deltaY.toFixed(
          2
        )}px (Threshold: ${this.thresholds.positionTolerance}px)`,
        suggestion:
          "This indicates a fundamental layout calculation error or transform issue",
        accuracy,
  ...
  private validateTransforms(node: ElementNode): TransformValidation | null {
    ...
    if (
      ...
      if (
        Math.abs(determinant) < this.thresholds.transformDeterminantThreshold
      ) {
        isDegenerate = true;
      ...
      if (scaleX < 0.01 || scaleY < 0.01) {
        warnings.push(
          "Extreme scaling detected that may cause rendering issues"
        );
      }

      if (scaleX > 100 || scaleY > 100) {
        warnings.push(
          "Very large scaling detected that may cause performance issues"
        );
      }
    }
    ...
    if (isDegenerate) {
      this.issues.push({
        ...
        suggestion:
          "This element may not render correctly in Figma due to invalid transform",
      });
    }
  ...
  private validateLayoutStructure(node: ElementNode): void {
    ...
    if (!hasAutoLayout && node.children.length >= 2) {
      ...
      if (childrenWithLayout.length >= 2) {
        ...
        const isLinearArrangement =
          this.detectLinearArrangement(childrenWithLayout);

        if (
          isLinearArrangement.isLinear &&
          isLinearArrangement.confidence > 0.8
        ) {
          ...
          this.issues.push({
            ...
            suggestion:
              "Consider using Auto Layout for more maintainable designs in Figma",
    ...
    if (hasAutoLayout && node.hasOverlappingElements) {
      this.issues.push({
        ...
        suggestion:
          "Overlapping elements in Auto Layout may not render as expected",
      });
    }
  }
  ...
  static generateSummary(report: ValidationReport): string {
    ...
    lines.push(
      `Validation Complete: ${report.valid ? "✅ PASSED" : "⚠️ ISSUES FOUND"}`
    );
    ...
    if (report.accuracyMetrics) {
      ...
      lines.push(
        `  • Average accuracy: ${report.accuracyMetrics.averagePositionAccuracy.toFixed(
          2
        )}px`
      );
      lines.push(
        `  • Worst position delta: ${report.accuracyMetrics.worstPositionDelta.toFixed(
          2
        )}px`
      );
      lines.push(
        `  • Average confidence: ${(
          report.accuracyMetrics.averageConfidence * 100
        ).toFixed(1)}%`
      );
      if (report.accuracyMetrics.coordinateSystemsUsed.length > 0) {
        lines.push(
          `  • Coordinate systems: ${report.accuracyMetrics.coordinateSystemsUsed.join(
            ", "
          )}`
        );
      }
    ...
    if (hasAnyStats) {
      ...
      if (report.stats.inaccuratePositions > 0) {
        lines.push(
          `  • Inaccurate positions: ${report.stats.inaccuratePositions}`
        );
      }
      if (report.stats.degenerateTransforms > 0) {
        lines.push(
          `  • Degenerate transforms: ${report.stats.degenerateTransforms}`
        );
      }
      if (report.stats.unsupported3DTransforms > 0) {
        lines.push(
          `  • 3D transforms: ${report.stats.unsupported3DTransforms}`
        );
      }
      if (report.stats.layoutStructureIssues > 0) {
        lines.push(
          `  • Layout structure issues: ${report.stats.layoutStructureIssues}`
        );
      }
    }
